...
# --- 1. INICJALIZACJA FLASK ---
app = Flask(__name__)

# --- 2. KONFIGURACJA AI & WP ---
...
# --- 3. SZABLON HTML (Zdefiniowany na sztywno jako zmienna globalna) ---
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="pl">
<head>
    ...
    <style>
        ...
        .badge-source { font-size: 0.7rem; text-transform: uppercase; background: #333; padding: 4px 8px; border-radius: 3px; }
    </style>
</head>
<body class="container py-5">
    <header class="mb-5 text-center">
        ...
        <button onclick="location.reload()" class="btn btn-outline-danger btn-sm">Skanuj Newsy</button>
    </header>

    <div id="articles">
        {% if not articles %}
            <div class="alert alert-dark text-center">Trwa pobieranie danych lub brak nowych treści...</div>
        {% endif %}
        {% for art in articles %}
        <div class="card shadow" id="card-{{ loop.index }}">
            <div class="card-body">
                <span class="badge-source mb-2 d-inline-block">{{ art.source_domain }}</span>
                <div class="mb-3">
                    <label class="form-label text-warning small">Tytuł (edytuj):</label>
                    <input type="text" id="title-{{ loop.index }}" class="form-control bg-dark text-white border-secondary" value="{{ art.ai_title }}">
                </div>
                <div class="mb-3">
                    <label class="form-label text-warning small">Treść (edytuj):</label>
        ...
        try:
            ...
            for link in list(dict.fromkeys(links))[:2]:
                try:
                    ...
                    title = art_soup.find('h1').get_text(strip=True) if art_soup.find('h1') else "News"
...
def index():
    ...
    # Renderowanie bez żadnych importów - używa globalnej zmiennej HTML_TEMPLATE
    data = get_all_news()
